...
private:
    ...
    void WithdrawItem(Player *player, uint32 entry) {
        ...
        std::string query = "SELECT amount FROM custom_reagent_bank WHERE character_id = " +
                            std::to_string(player->GetGUID().GetCounter()) + " AND item_entry = " + std::to_string(
                                entry);
        QueryResult result = CharacterDatabase.Query(
            "SELECT amount FROM custom_reagent_bank WHERE character_id = " +
            std::to_string(player->GetGUID().GetCounter()) + " AND item_entry = " + std::to_string(entry));
        if (result) {
            ...
            if (storedAmount <= stackSize) {
                ...
                if (msg == EQUIP_ERR_OK) {
                    CharacterDatabase.Execute(
                        "DELETE FROM custom_reagent_bank WHERE character_id = {} AND item_entry = {}",
                ...
                if (msg == EQUIP_ERR_OK) {
                    CharacterDatabase.Execute(
                        "UPDATE custom_reagent_bank SET amount = {} WHERE character_id = {} AND item_entry = {}",
                        storedAmount - stackSize, player->GetGUID().GetCounter(), entry);
                    Item *item = player->StoreNewItem(dest, entry, true);
    ...
    void UpdateItemCount(std::map<uint32, uint32> &entryToAmountMap, std::map<uint32, uint32> &entryToSubclassMap,
                         Item *pItem, Player *player, uint32 bagSlot, uint32 itemSlot) {
        uint32 count = pItem->GetCount();
        ItemTemplate const *itemTemplate = pItem->GetTemplate();

        if (!(itemTemplate->Class == ITEM_CLASS_TRADE_GOODS || itemTemplate->Class == ITEM_CLASS_GEM) || itemTemplate->
            GetMaxStackSize() == 1)
            return;
    ...
    void DepositAllReagents(Player *player) {
        ...
        std::string query = "SELECT item_entry, item_subclass, amount FROM custom_reagent_bank WHERE character_id = " +
            ...
            [=, this](QueryResult result) {
                ...
                if (entryToAmountMap.size() != 0) {
                    ...
                    for (std::pair<uint32, uint32> mapEntry: entryToAmountMap) {
                        ...
                        trans->Append(
                            "REPLACE INTO custom_reagent_bank (character_id, item_entry, item_subclass, amount) VALUES ({}, {}, {}, {})",
                            player->GetGUID().GetCounter(), itemEntry, itemSubclass, itemAmount);
                    }
...
public:
    ...
    bool OnGossipHello(Player *player, Creature *creature) override {
        switch (player->GetSession()->GetSessionDbcLocale()) {
            case LOCALE_frFR: {
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(4359, 30, 30, -18, 0) + "Pièces détachées",
                                 ITEM_SUBCLASS_PARTS, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(4358, 30, 30, -18, 0) + "Explosifs",
                                 ITEM_SUBCLASS_EXPLOSIVES, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(4388, 30, 30, -18, 0) + "Appareils",
                                 ITEM_SUBCLASS_DEVICES, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(1206, 30, 30, -18, 0) + "Joaillerie",
                                 ITEM_SUBCLASS_JEWELCRAFTING, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(2589, 30, 30, -18, 0) + "Étoffe",
                                 ITEM_SUBCLASS_CLOTH, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(2318, 30, 30, -18, 0) + "Cuir",
                                 ITEM_SUBCLASS_LEATHER, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(2772, 30, 30, -18, 0) + "Métal et pierre",
                                 ITEM_SUBCLASS_METAL_STONE, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(12208, 30, 30, -18, 0) + "Viande",
                                 ITEM_SUBCLASS_MEAT, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(2453, 30, 30, -18, 0) + "Herbe",
                                 ITEM_SUBCLASS_HERB, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(7068, 30, 30, -18, 0) + "Élémentaire",
                                 ITEM_SUBCLASS_ELEMENTAL, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(10940, 30, 30, -18, 0) + "Enchantement",
                                 ITEM_SUBCLASS_ENCHANTING, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 GetItemIcon(23572, 30, 30, -18, 0) + "Matériau du Néant", ITEM_SUBCLASS_MATERIAL, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 GetItemIcon(2604, 30, 30, -18, 0) + "Autres biens commerciaux",
                                 ITEM_SUBCLASS_TRADE_GOODS_OTHER, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                 GetItemIcon(38682, 30, 30, -18, 0) + "Parchemin d'armure",
                                 ITEM_SUBCLASS_ARMOR_ENCHANTMENT, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(39349, 30, 30, -18, 0) + "Parchemin d'arme",
                                 ITEM_SUBCLASS_WEAPON_ENCHANTMENT, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "Déposer tous les composants", DEPOSIT_ALL_REAGENTS, 0);
                SendGossipMenuFor(player, NPC_TEXT_ID, creature->GetGUID());
                return true;
            }
            default: {
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(4359, 30, 30, -18, 0) + "Parts",
                                 ITEM_SUBCLASS_PARTS, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(4358, 30, 30, -18, 0) + "Explosives",
                                 ITEM_SUBCLASS_EXPLOSIVES, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(4388, 30, 30, -18, 0) + "Devices",
                                 ITEM_SUBCLASS_DEVICES, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(1206, 30, 30, -18, 0) + "Jewelcrafting",
                                 ITEM_SUBCLASS_JEWELCRAFTING, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(2589, 30, 30, -18, 0) + "Cloth",
                                 ITEM_SUBCLASS_CLOTH, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(2318, 30, 30, -18, 0) + "Leather",
                                 ITEM_SUBCLASS_LEATHER, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(2772, 30, 30, -18, 0) + "Metal & Stone",
                                 ITEM_SUBCLASS_METAL_STONE, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(12208, 30, 30, -18, 0) + "Meat",
                                 ITEM_SUBCLASS_MEAT,
                                 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(2453, 30, 30, -18, 0) + "Herb",
                                 ITEM_SUBCLASS_HERB,
                                 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(7068, 30, 30, -18, 0) + "Elemental",
                                 ITEM_SUBCLASS_ELEMENTAL, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(10940, 30, 30, -18, 0) + "Enchanting",
                                 ITEM_SUBCLASS_ENCHANTING, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(23572, 30, 30, -18, 0) + "Nether Material",
                                 ITEM_SUBCLASS_MATERIAL, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(2604, 30, 30, -18, 0) + "Other Trade Goods",
                                 ITEM_SUBCLASS_TRADE_GOODS_OTHER, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(38682, 30, 30, -18, 0) + "Armor Vellum",
                                 ITEM_SUBCLASS_ARMOR_ENCHANTMENT, 0);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, GetItemIcon(39349, 30, 30, -18, 0) + "Weapon Vellum",
                                 ITEM_SUBCLASS_WEAPON_ENCHANTMENT, 0);
    ...
    void ShowReagentItems(Player *player, Creature *creature, uint32 item_subclass, uint16 gossipPageNumber) {
        WorldSession *session = player->GetSession();
        std::string query = "SELECT item_entry, amount FROM custom_reagent_bank WHERE character_id = " + std::to_string(
                                player->GetGUID().GetCounter()) + " AND item_subclass = " +
            ...
            [=, this](QueryResult result) {
                ...
                for (uint32 i = startValue; i <= endValue; i++) {
                    ...
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG,
                                     ...
                                     std::to_string(entryToAmountMap.find(itemEntry)->second) + ")", itemEntry,
                                     gossipPageNumber);
                }
                ...
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/Ability_Spy:30:30:-18:0|tBack...",
                                 MAIN_MENU, 0);
                SendGossipMenuFor(player, NPC_TEXT_ID, creature->GetGUID());
            }));
    }
